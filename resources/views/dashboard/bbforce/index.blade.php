        ...
        <div class="page-header">
            ...
            <div>
                <ol class="breadcrumb">
                    ...
                    <li class="breadcrumb-item active" aria-current="page">Bbforce</li>
                </ol>
            </div>
        </div>
        <!-- PAGE-HEADER END -->
            ...
            <div class="col-lg-12">
                <div class="card">
                    <div class="d-flex justify-content-between card-header">
                        <h3 class="card-title">Bbforce Members</h3>
                    </div>
                    <div class="card-body">
                            ...
                            <table class="table table-bordered text-nowrap border-bottom" id="basic-datatable">
                                ...
                                <tbody>
                                        @php
                                            $id = 1;
                                        @endphp
                                    @foreach ($bbforceUsers as $bbforceUser)
                                        <tr>
                                            <td>{{ $id++ }}</td>
                                            <td>{{ $bbforceUser->name }}</td>
                                            <td>{{ $bbforceUser->username }}</td>
                                            <td>{{ $bbforceUser->email }}</td>
                                            <td>{{ $bbforceUser->created_at->diffForHumans() }}</td>
                                            <td>
                                                {{-- group edit and delete Icon --}}
                                                <div class="btn-group">
                                                    @if($bbforceUser->is_banned == 0)
                                                    <form method="post" action="{{ route('users.ban', $bbforceUser->id) }}">
                                                        @csrf
                                                    ...
                                                    @else
                                                    <form method="post" action="{{ route('users.unban', $bbforceUser->id) }}">
                                                        @csrf
                                                    ...
                                                    <form method="post" action="{{ route('users.destroy', $bbforceUser->id) }}">
                                                        @method('delete')
                                                        @csrf
